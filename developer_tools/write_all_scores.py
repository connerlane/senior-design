...
table = []
with open("all_predicted_scores.csv", "w") as f:
    f.write(",".join(["Name"] + [q + "_pred" for q in labels[2:]]) + "\n")
for i in range(NUM_PARTITIONS):
    ...
    y_train = []
    for entry in train:
        x_train.append(extract_features(entry[1])[1])
        y_train.append(np.array([float(e) for e in entry[3:]]))
    ...
    for entry in validation:
        ...
        print(entry[0])
        x_val.append(extract_features(entry[1])[1])
        y_val.append(np.array([float(e) for e in entry[3:]]))
    pred = reg_model.predict(x_val)

    with open("all_predicted_scores.csv", "a") as f:
        for i, p in enumerate(pred):
            f.write(",".join([validation[i][0]] + [str(q) for q in p]) + "\n")
